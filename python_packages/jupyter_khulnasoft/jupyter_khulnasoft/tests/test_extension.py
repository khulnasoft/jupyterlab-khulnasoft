def test_serverextension_path(app):
    import jupyter_khulnasoft

    paths = jupyter_khulnasoft._jupyter_server_extension_paths()
    for path in paths:
...
def test_serverextension(app):
    app.initialize(
        ["--NotebookApp.nbserver_extensions={'jupyter_khulnasoft.serverextension': True}"]
    )
