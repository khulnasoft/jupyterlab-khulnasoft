...
from jupyter_khulnasoft import lsp_message_listener


@pytest.mark.parametrize("bad_string", ["not-a-function", "jupyter_khulnasoft.__version__"])
@pytest.mark.asyncio
...
async def test_listeners(known_server, handlers, jsonrpc_init_msg):
    ...
    manager.all_listeners = ["jupyter_khulnasoft.tests.listener.dummy_listener"]
