...
BUILDING_DOCS = os.environ.get("JUPYTER_KHULNASOFT_BUILDING_DOCS") is not None
...
class KhulnasoftShellSpec(SpecBase):
    """Helper for a language server spec for Khulnasoft."""

    def locate_khulnasoftlsp(self):
        """Detect if khulnasoft-lsp is installed and return the installation path."""
        path = ''
        if os.name == 'nt':
            path = 'C:\\Program Files\\Khulnasoft\\khulnasoft-lsp.exe'
        elif sys.platform.startswith('linux'):
            path = osp.expanduser('~/.local/share/khulnasoft/current/khulnasoft-lsp')
        elif sys.platform == 'darwin':
            path = self.locate_khulnasoftlsp_darwin()
        return path

    def locate_khulnasoftlsp_darwin(self):
        """
        Looks up where khulnasoft-lsp is installed on macOS systems. The bundle ID
        is checked first and if nothing is found or an error occurs, the
        default path is used.
        """
        default_path = '/Applications/Khulnasoft.app'
        path = ''
        bundle_path = "/Contents/MacOS/khulnasoft-lsp"
        try:
    ...
    cmd = property(locate_khulnasoftlsp)
