...
interface IKhulnasoftLSPCompletionItem extends CompletionItem {
  hint: string;
}

export class KhulnasoftConnector extends DataConnector<
...
> {
  ...
  protected options: KhulnasoftConnector.IOptions;
  ...
  private icon: LabIcon = new LabIcon({
    name: 'jupyterlab-khulnasoft:completion-icon',
    svgstr: khulnasoftLogo
  });

  /**
   * Create a new Khulnasoft connector for completion requests.
   *
   * @param options - The instantiation options for the Khulnasoft connector.
   */
  constructor(options: KhulnasoftConnector.IOptions) {
    super();
  ...
  fetch(
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (
        ...
        console.log(
          '[Khulnasoft][Completer] Suppressing completer auto-invoke in',
          token.type
        );
        return;
      }
      ...
      if (!start || !end) {
        console.log('[Khulnasoft][Completer] No start or end position found');
        resolve(KhulnasoftConnector.EmptyICompletionItemsReply);
        return;
      }
      ...
      const khulnasoftPromise = async () => {
        try {
          return await this.fetch_khulnasoft(
            token,
            typed_character,
            virtual_start,
            virtual_end,
      ...
      const kernelPromise = () => {
        ...
        if (
          ...
          return KhulnasoftConnector.EmptyIReply;
        }
      ...
      const kernelTimeoutPromise = () => {
        const timeout = new Promise<CompletionHandler.IReply>(resolve => {
          setTimeout(resolve, 500, KhulnasoftConnector.EmptyIReply);
        });
        return Promise.race([timeout, kernelPromise()]);
      };

      const [kernel, khulnasoft] = await Promise.all([
        isManual ? kernelPromise() : kernelTimeoutPromise(),
        khulnasoftPromise()
      ]);
      const merged = this.merge_replies(kernel, khulnasoft);
  ...
  async fetch_khulnasoft(
    ...
    if (!connection) {
      console.log('[Khulnasoft][Completer] No LSP Connection found');
      return KhulnasoftConnector.EmptyICompletionItemsReply;
    }
    ...
    console.log('[Khulnasoft][Completer] Fetched', lspCompletionItems);
    ...
    let lineText = document
      ...
      .join('');
    (lspCompletionItems as IKhulnasoftLSPCompletionItem[]).forEach(match => {
      ...
      if (range) {
        if (range.start.character < start.ch) {
          ...
          if (insertion.startsWith(preToken)) {
            ...
            console.log(
              '[Khulnasoft][Completer] Disposing of un-insertable completion: %s',
      ...
      if (insertion !== match.insertText) {
        console.log(
          '[Khulnasoft][Completer] Trimmed %s => %s',
          match.insertText,
          insertion
        );
  ...
  private merge_replies(
    ...
    khulnasoftReply: CompletionHandler.ICompletionItemsReply
  ): CompletionHandler.ICompletionItemsReply {
    const newKernelReply = this.transform(kernelReply);
    if (!newKernelReply.items.length) {
      return khulnasoftReply;
    }
    if (!khulnasoftReply.items.length) {
    ...
    console.log('[Khulnasoft]: Merging', khulnasoftReply, newKernelReply);
    if (khulnasoftReply.start === -1) {
      // Use kernel replacement range
      return {
        ...kernelReply,
        items: khulnasoftReply.items.concat(newKernelReply.items)
      };
    } else {
      // Use khulnasoft replacement range
      return {
        ...khulnasoftReply,
        items: khulnasoftReply.items.concat(newKernelReply.items)
      };
...
/**
 * A namespace for Khulnasoft connector statics.
 */
export namespace KhulnasoftConnector {
  /**
   * Interface that adds a noFilter flag to an ICompletionItem
   */
  export interface IKhulnasoftCompletionItem
