...
import { KhulnasoftConnector } from './components/completion';
import { FreeTooltip } from './components/free_tooltip';
import { KhulnasoftCompleter } from './khulnasoft_completer';
import { KhulnasoftModel } from './khulnasoft_model';
...
export abstract class JupyterLabWidgetAdapter
  ...
  protected document_connected: Signal<
    ...
    IDocumentConnectionData
  >;
  protected abstract current_completion_connector: KhulnasoftConnector;
  ...
  async invoke_completer(kind: CompletionTriggerKind) {
    if (this.completion_handler) {
      ...
      let model: KhulnasoftModel;
      try {
        model = this.completion_handler.completer.model as KhulnasoftModel;
      } catch (e) {
        console.log('Could not cast model into KhulnasoftModel', e);
        return;
      }
  ...
  registerKhulnasoftModules(
    ...
    if (handler instanceof CompletionHandler) {
      ...
      const khulnasoftModel = new KhulnasoftModel();
      this.completion_handler.completer.model = khulnasoftModel;

      const khulnasoftCompleter = new KhulnasoftCompleter(
        {
          editor: editor,
          model: khulnasoftModel
        },
        state
      );
      try {
